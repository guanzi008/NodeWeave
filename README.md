...
- 这些窗口现在都可以通过控制面环境变量调节，包括在线判定、endpoint freshness、按失败类型区分的 cooldown、attempt window、`manual_recover` 触发时机，以及 `relay_active` / `manual_recover` 各自独立的 lead/window/burst profile
