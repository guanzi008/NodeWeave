...
func printUsage() {
	fmt.Fprintln(os.Stderr, "usage: windows-agent <init-config|enroll|run|status|bootstrap-status|runtime-status|heartbeat|serial-forward-status|serial-forward-report|usb-forward-status|usb-forward-report>")
}
