...
| USB / 串口 | 远程设备转发 | 串口基础层已开始 | `packages/runtime/go/forwarding/serial` | usbip、libusb |
| Windows/macOS/mobile | 多平台客户端 | Windows 基础骨架已开始 | `clients/windows-agent`、`clients/windows-cli`，其余平台仍为空壳 | 平台壳层和系统网络扩展 |
